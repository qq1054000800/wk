...
function add_black(obj_uid,obj_username) {
	if (check_user_login()) {
		var url='index.php?do=ajax&view=ajax&ac=add_black';
		$.post(url,{obj_uid:obj_uid,obj_username:obj_username},function(json){
			if(json.status==1){
 ...
 * fid 附件、文件编号
 */
function addDesc(fid) {
if(check_user_login()){

	var url = 'index.php?do=ajax&view=filedesc&op=add&fid='+ fid;
		showWindow('filedesc',encodeURI(url));return false;
}
}
/**
 * 查看附件、文件描述
 ...
 * fid 附件、文件编号
 */
function viewDesc(fid) {
if(check_user_login()){

	var url = 'index.php?do=ajax&view=filedesc&op=view&fid='+ fid;
		showWindow('filedesc',encodeURI(url));return false;
}
}
/**
...
function qx_collect(obj){
	showDialog("确定取消收藏吗？",'confirm',"操作提示","formSub('"+obj.href+"','url',false);");
	return false;
}
...
function focus_tag(tag_id){
	if (check_user_login()) {
		$.post('index.php?do=login_index&ac=focus_tags',{tag_id:tag_id},function(json){
			if(json.status == 1){
					showDialog('关注成功','right','操作提示','window.location.reload()');
				}
...
function qxfocus_tags(tag_id){
	if (check_user_login()) {
		$.post('index.php?do=login_index&ac=qxfocus_tags',{tag_id:tag_id},function(json){
			if(json.status == 1){
					showDialog('取消成功','right','操作提示','window.location.reload()');
				}
...
function setCopy(text, msg,type){
	if(BROWSER.ie) {
		...
		var msg = '<div class="c"><div style="width: 400px; text-align: center; text-decoration:underline;">'+L.click_copy+'</div>' +
...
function sendMsg(to_uid){
        if(check_user_login()){
        showWindow('msg','index.php?do=login_index&view=seller&ac=sendmsg&to_uid='+to_uid);return false;
        }
}
 ...
 * 复制链接
 */
function clip_href(obj,text){
    var t_str=text==''||!text?location.href:text;
        $(obj).zclip({
        path:'resource/js/jqplugins/zclip/ZeroClipboard.swf',
        copy:function(){return t_str;},
        afterCopy:function(){
         var msginfo = '<div class="c"><div style="width: 600px; text-align: center;  position:relative;">'+
		'复制成功，您可以分享给好友了!' + t_str+'</div></div>';
           showDialog(msginfo, 'info','推荐');return false;
