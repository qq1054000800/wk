...
class RBAC {
	...
	static function saveAccessList($authId = null) {
		if (!$_SESSION['TOPadmin']) {
			$_SESSION['_ACCESS_LIST'] = RBAC::getAccessList($authId ? $authId : $_SESSION['id']);
		}
		return;
	}
	...
	static public function getAccessList($authId) {
		...
		$sql    =
			"select node.id,node.name from ".$table['role']." as role,".$table['user']." as user,".$table['access']
			." as access ,".$table['node']." as node "
		...
		foreach ($apps as $app) {
			...
			foreach ($modules as $key => $module) {
				...
				if ('PUBLIC' == strtoupper($moduleName)) {
					$sql = "select node.id,node.name from ".$table['role']." as role,".$table['user']." as user,"
					       .$table['access']." as access ,".$table['node']." as node "
					       ."where user.user_id='{$authId}' and user.role_id=role.id and ( access.role_id=role.id  or (access.role_id=role.pid and role.pid!=0 ) ) and role.status=1 and access.node_id=node.id and node.level=3 and node.pid={$module_id} and node.status=1";
					$rs  = $db->query($sql);
					foreach ($rs as $a) {
	...
	static public function getModuleAccessList($authId, $module) {
		...
		$sql    = "select access.node_id from ".$table['role']." as role,".$table['user']." as user,".$table['access']
		          ." as access "
