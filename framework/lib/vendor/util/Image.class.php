...
class Image {
		...
		if ($info !== false) {
			...
			if ('png' == $type) {
				...
				if ($trnprt_indx >= 0) {
					...
					$trnprt_indx  =
						imagecolorallocate($thumbImg, $trnprt_color['red'], $trnprt_color['green'], $trnprt_color['blue']);
	 ...
	 * @static
	 * @access public
	 * @param string $length  位数 验证码的长度 默认4位
	 * @param string $mode    验证字符串的类型，默认为数字，其他支持类型有0 字母 1 数字 2 大写字母 3 小写字母 4中文 5混合
	 * @param string $type    验证码的图片类型，默认为png
	 * @param string $width   验证码的宽度，默认会自动根据验证码长度自动计算
	 * @param string $height  验证码的高度，默认为22
	 * @return string
	 */
	static function buildImageVerify($length = 4, $mode = 1, $type = 'png', $width = 48, $height = 22, $verifyName = 'verify') {
		include_once VENDOR_PATH.'/util/String.class.php';
		$randval               = String::randString($length, $mode);
		$_SESSION[$verifyName] = Z(strtolower($randval));//仅仅使用小写
		$width                 = ($length * 10 + 10) > $width ? $length * 10 + 10 : $width;
		if ($type != 'gif' && function_exists('imagecreatetruecolor')) {
			$im = imagecreatetruecolor($width, $height);//创建真彩图层
		} else {
			$im = imagecreate($width, $height);//创建图层
		}
		$r   = Array(
			225,
			255,
	...
	static function GBVerify($length = 4, $type = 'png', $width = 180, $height = 50, $fontface = 'simhei.ttf', $verifyName = 'verify') {
		...
		for ($i = 0; $i < $length; $i++) {
			...
			imagettftext($im, mt_rand(16, 20), mt_rand(-60, 60),
				40 * $i + 20, mt_rand(30, 35), $fontcolor, $fontface, $codex);
		}
		Image::output($im, $type);
	}
