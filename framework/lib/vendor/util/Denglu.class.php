...
class Denglu {
	...
	function latestComment($count) {
		return $this->callApi('latestComment', array(
			'appid' => $this->appID,
			'count' => $count
		), 12);
	}
	...
	function getComments($commentid, $count = 50) {
		return $this->callApi('getComments', array(
			'appid'     => $this->appID,
			'commentid' => $commentid,
			'count'     => $count
		));
	...
	function getCommentState($time) {
		return $this->callApi('getCommentState', array(
			'appid' => $this->appID,
			'time'  => $time
		));
	}
	...
	function getBind($muid, $uid = null) {
		if (empty($muid) || !isset($muid)) {
			return $this->callApi('getBind', array(
				'appid' => $this->appID,
				'uid'   => $uid
			));
		}
	...
	function getInvite($muid, $uid = null) {
		if (empty($muid) || !isset($muid)) {
			return $this->callApi('getInvite', array(
				'appid' => $this->appID,
				'uid'   => $uid
			));
		}
	...
	function getRecommend($muid, $uid = null) {
		if (empty($muid) || !isset($muid)) {
			return $this->callApi('getRecommend', array(
				'appid' => $this->appID,
				'uid'   => $uid
			));
		}
	...
	function sendInvite($invitemuids, $muid, $uid = null) {
		if (empty($muid) || !isset($muid)) {
			return $this->callApi('sendInvite', array(
				'appid'      => $this->appID,
				'uid'        => $uid,
				'invitemuid' => $invitemuids
			));
		}
		return $this->callApi('sendInvite', array(
			'appid'      => $this->appID,
	...
	function bind($mediaUID, $uid, $uname, $uemail) {
		return $this->callApi('bind', array(
			'appid'  => $this->appID,
			'muid'   => $mediaUID,
			'uid'    => $uid,
			'uname'  => $uname,
	...
	function unbind($mediaUID) {
		return $this->callApi('unbind', array(
			'appid' => $this->appID,
			'muid'  => $mediaUID
		));
	}
	...
	function sendLoginFeed($mediaUserID) {
		return $this->callApi('login', array(
			'muid'  => $mediaUserID,
			'appid' => $this->appID
		));
	}
	...
	function share($mediaUserID, $content, $url, $uid, $imageurl = '', $videourl = '', $param1 = '') {
		return $this->callApi('share', array(
			'appid'    => $this->appID,
			'muid'     => $mediaUserID,
			'uid'      => $uid,
			'content'  => $content,
	...
	function unbindAll($uid) {
		return $this->callApi('unbindAll', array(
			'uid'   => $uid,
			'appid' => $this->appID
		));
	}
	...
	protected function makeRequest($url, $post = '') {
		...
		if (function_exists('fsockopen')) {
			...
			return array(
				'errorCode'        => 1,
				'errorDescription' => 'Functions "fsockopen" and "pfsockopen" are not exists!'
			);
		}
